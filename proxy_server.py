#!/usr/bin/env python3
"""
Ollama Proxy Server v4 — gemma4:e4b — RAM-Resident Mode
=========================================================
Key improvements over v3:
  • Model is pinned in RAM via keep_alive=-1 on EVERY request
  • Background RAM-keepalive pings every KEEPALIVE_INTERVAL seconds
  • Connection pool is tuned with TCP keepalives to prevent econnreset
  • /api/ps endpoint proxied so clients can verify model is loaded
  • Auto-reload: if model is evicted from RAM, a reload is triggered
  • Full debug logging with request/response timing
  • All original endpoints retained and hardened
  • Retry logic on every Ollama call (3 attempts, exponential backoff)
  • CORS fully open for all clients
  • Trailing slash normalisation middleware
  • Model alias table: any name resolves to MODEL
"""

import os, sys, json, time, uuid, threading, random, subprocess, shutil, logging
...
# Keep model in RAM forever
OLLAMA_KEEP_ALIVE = -1

# Alias table — any of these model names are treated as MODEL
MODEL_ALIASES = {
    MODEL, MODEL.split(":")[0],
    "gemma4", "gemma4:e4b", "gemma", "default", "local",
    # Legacy aliases from previous versions
...
def _make_session() -> requests.Session:
    ...
    # TCP-level keepalive so long-running streams never hit econnreset
...
def ensure_model_in_ram():
    """
    Send a minimal generation request with keep_alive=-1.
    This forces Ollama to load the model weights into RAM and keep them there.
    """
...
def _activity_logger_thread():
    """Log system metrics every LOG_INTERVAL seconds."""
    import psutil
    while True:
        try:
...
def _model_keepalive_thread():
    """
    Ping Ollama every KEEPALIVE_INT seconds.
    - /api/tags  — verifies Ollama is alive
    - /api/ps    — checks if model is still in RAM
    - If model evicted, re-run ensure_model_in_ram()
    """
    s = _make_session()
    ...
    while True:
        try:
            ...
            # 2. Check /api/ps — is model still loaded?
            try:
                ...
                # /api/ps might not exist on older Ollama; ignore
                pass
...
def _msgs_to_prompt(messages: List[Dict]) -> str:
    """Flatten messages → single prompt string for /api/generate fallback."""
    parts = []
    for m in messages:
        role    = m.get("role", "user")
        content = _content_to_str(m.get("content") or "")
...
def _resolve_model(name: Optional[str]) -> str:
    """Resolve any client-provided model name to MODEL."""
    if not name:
        return MODEL
    ...
    for alias in MODEL_ALIASES:
        if alias and name in alias:
            return MODEL
    return MODEL   # always use our MODEL
...
def _stream_anthropic(messages, tools, max_tokens, temp):
    ...
    yield (
        ...
        f"data: {json.dumps({'type':'message_start','message':{'id':mid,'type':'message','role':'assistant','content':[],'model':MODEL,'stop_reason':None,'usage':{'input_tokens':0,'output_tokens':0}}})}\n\n"
    )
    yield (
    ...
    for raw in resp.iter_lines():
        ...
        if done:
            ...
            for i, tc in enumerate(tcs, 1):
                ...
                yield (
                    f"event: content_block_start\n"
                    f"data: {json.dumps({'type':'content_block_start','index':i,'content_block':{'type':'tool_use','id':tid,'name':f.get('name',''),'input':{}}})}\n\n"
                )
                yield (
                    f"event: content_block_delta\n"
                    f"data: {json.dumps({'type':'content_block_delta','index':i,'delta':{'type':'input_json_delta','partial_json':json.dumps(args if isinstance(args,dict) else {})}})}\n\n"
                )
                yield f"event: content_block_stop\ndata: {json.dumps({'type':'content_block_stop','index':i})}\n\n"
            yield (
                f"event: message_delta\n"
                f"data: {json.dumps({'type':'message_delta','delta':{'stop_reason':stop,'stop_sequence':None},'usage':{'output_tokens':0}})}\n\n"
            )
...
app = FastAPI(title="Ollama Proxy v4 — gemma4:e4b", version="4.0.0", lifespan=lifespan)
...
async def root():
    """
    OpenAI-style model discovery doc.
    Clients probing GET / for auto-detection will find the model here.
    """
    return {
        "object": "list",
        "data": [{
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            loaded = (MODEL in names or
                      any(n.split(":")[0] == MODEL.split(":")[0] for n in names))
            # Also check /api/ps for in-RAM status
            ram_loaded = False
            try:
                ps_r = ollama.get(f"{OLLAMA_BASE}/api/ps", timeout=3)
                if ps_r.status_code == 200:
                    ps_models = [m.get("name","") for m in ps_r.json().get("models", [])]
                    ram_loaded = any(MODEL in n or n.split(":")[0] == MODEL.split(":")[0]
                                     for n in ps_models)
            except Exception:
                ram_loaded = None  # unknown
        ...
        return {"status": "down", "model": MODEL, "error": str(e), "ollama_ok": False}
    return {"status": "down", "model": MODEL, "ollama_ok": False}
        ...
        if r.status_code == 200:
            data = [
                {"id": m.get("name",""), "object": "model",
                 "created": int(time.time()), "owned_by": "ollama",
                 "root": m.get("name",""), "parent": None, "permission": []}
...
async def ollama_tags():
    """Mirror /api/tags — always includes MODEL even if Ollama is unreachable."""
    try:
        r = ollama.get(f"{OLLAMA_BASE}/api/tags", timeout=5)
        r.raise_for_status()
        return r.json()
...
async def ollama_version():
    """Always returns >= 0.1.16 so clients never show the /api/chat warning."""
    try:
...
@app.get("/api/ps")
async def ollama_ps():
    """Proxy /api/ps — shows models currently loaded in RAM."""
    try:
        r = ollama.get(f"{OLLAMA_BASE}/api/ps", timeout=5)
        r.raise_for_status()
        return r.json()
    except Exception:
        # Return a synthetic response so clients don't break
        return {"models": []}
...
async def execute_tool(request: Request):
    ...
    return {
        "tool":   name,
        "result": json.loads(tool_executor.execute(name, args)),
    }
...
async def reload_model_into_ram():
    """Force the model to reload into RAM. Useful after eviction."""
...
async def catch_all(path: str, request: Request):
    ...
    return JSONResponse({
        ...
        "hint": (
            "Use /v1/chat/completions (OpenAI), "
            "/v1/messages (Anthropic), "
            "or /api/chat (raw Ollama)"
        ),
        "available": [
            "/", "/health", "/v1/models", "/v1/tools",
            "/v1/chat/completions", "/v1/messages",
            "/v1/tool/execute", "/v1/model/reload",
...
if __name__ == "__main__":
    print("=" * 72, flush=True)
    print("  Ollama Proxy v4 — gemma4:e4b — RAM-Resident Mode", flush=True)
    print("=" * 72, flush=True)
    print(f"  Model:       {MODEL}", flush=True)
    print(f"  Ollama:      {OLLAMA_BASE}", flush=True)
    print(f"  Port:        {PROXY_PORT}", flush=True)
    ...
    print("", flush=True)
    print("  Client options (all work):", flush=True)
    print("    apiBase = https://…trycloudflare.com/v1   ← OpenAI-compatible", flush=True)
    print("    apiBase = https://…trycloudflare.com      ← raw Ollama", flush=True)
    print("    apiBase = https://…trycloudflare.com/v1/messages ← Anthropic", flush=True)
    print(f"   model   = {MODEL}", flush=True)
    print("=" * 72, flush=True)
    ...
    uvicorn.run(
        ...
        port=PROXY_PORT,
        log_level="warning",
        timeout_keep_alive=600,     # 10-min HTTP keepalive
        h11_max_incomplete_event_size=16 * 1024 * 1024,
        access_log=False,           # proxy does its own logging
    )
